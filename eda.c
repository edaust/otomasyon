...
struct araba_ozellikler{
	...
	float satis_fiyat;
	float araba_kar;
	float motor_hacmi; // ürün: 1.4---1.6---2.0---3.0
	int yakit_tipi; // 1 ise benzin 2 ise benzin ve lpg 3 ise dizel
	int kasa_tipi; //1 ise hatchback 2 ise sedan
...
void ekle(){
	...
	if(araba_ptr==NULL){
		flag+=1;
		araba_ptr = (araba *)calloc(sizeof(araba),1);
		printf("Araba Marka: ");
		...
		araba_ptr->araba_ozellik = calloc(sizeof(*araba_ptr->araba_ozellik),araba_ptr->araba_adet);
		int i;
		...
		flag+=1;
		araba_ptr = (araba *)realloc(araba_ptr, flag * sizeof(araba));
...
int main(int argc, char *argv[]) {
	...
	while(1){
		system("COLOR F2");
		system("cls");
		printf("\nGALERI YONETIM OTOMASYONUNA HOSGELDINIZ..");
		printf("\n1-)Araba Ekle");
